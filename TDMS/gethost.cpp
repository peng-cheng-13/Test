...
char* gethost_c(char* argv){
    ...
    if(status != JNI_ERR){
	//printf("create JVM successfully\n");
	cls = env->FindClass("InitTDMS");
	if(cls !=0){
	    //printf("find class successfully\n");

	    //get the jmethodID	    
	    mid=env->GetStaticMethodID(cls, "gethost", "()Lalluxio/client/file/FileSystem;");
	    if(mid !=0){
		    ...
		    test = "get Filesystem successfully";
		    printf("get Filesystem successfully\n");
		    
	    }
	    else
		printf("get static method Failed\n");
	}
	jvm->DestroyJavaVM(); 
	return test;
    }else{
